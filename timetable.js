...
function typeForm(id,val,slot,clas)
{
    ...
    document.getElementById("form").innerHTML= 
                ...
                <input type="button" onclick="trainList()" value="Trainer list"><select id="listt"></select><br />
...
function trainList()
{
    ...
    let rows= `<select id="trlist onchange="setDetails1()">`;
    for(let i=0;i<arr.length;i++)
    { rows+= `<option value="${arr[i]}">${arr[i]}</option>`
    }
      document.getElementById("listt").innerHTML = `${rows}
                                                </select>`                                         
}
function setDetails1(){
    let tlist = document.getElementById("listt").value;
    let listArr = JSON.parse(localStorage.getItem(tlist));
    if(arr.subject.includes(document.getElementById("subject").value))
    {
       tabShow(tlist,document.getElementById("subject").value); 
    }
}
